	...
	public static String[] leerArchivo(String nombreCompletoArchivo){
		...
		try {
			...
			while (lineaActual != null){
                                Pattern pattern = Pattern.compile("([a-záéíóúñ\\s]*)"); //ReGex que elimina caracteres que no sean espacios ni alfabeticos
                                Matcher match = pattern.matcher(lineaActual);    
                                if(match.find())
                                    lineaActual = match.group(1);
